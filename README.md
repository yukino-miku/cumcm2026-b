...
新增方案一[扇形清空版](docs/第三问/扇形版_实现与运行.md)，独立入口`scripts/run_q3_scheme1_sector.py`：保留七固定站，当前扇形清空后才推进，优先复用智能站、仅实际覆盖不足时补站。全库200项测试通过，60场景两版本共120次构造运行均全清。独立验证集末段路程4.68→1.00公里，但平均总时间增加10.95%、总路程基本持平，保留独立版本供测试。见[中文验证报告、轨迹与流程图](docs/第三问/扇形版_验证与结果.md)。
