...
2026 年全国大学生数学建模竞赛 B 题项目。7 个原始文件已归档；第一问、第二问已完成，第三问两套方案已实现并通过本地闭环与同场景对照，配套中文说明、图表和论文备用资料。
...
**第三问入口：[阅读导航](docs/第三问/阅读导航.md)**。包含七站自适应搜索、固定双覆盖搜索、定位清除与HTTP客户端，两种运行入口分别为`scripts/run_q3_scheme1.py`和`scripts/run_q3_scheme2.py`。当前默认机制及复现方法如下。

**当前默认已加入两方案顺路清除和路线规划，方案二同时停止已定位频道的重复固定检测。** 全库174项测试通过；运行说明、原版恢复参数及当前六设置对照见[改进版使用说明](docs/第三问/路线调度_实现与使用说明.md)和[验证结果](docs/第三问/路线调度_验证与结果.md)。当前完整复现入口为`pwsh -NoProfile -File scripts/run-q3-routes.ps1`；旧构建脚本继续保留历史基线。

最新[两组实际演练评估](docs/第三问/两组演练评估_s1_s2.md)已核对用户提供的s1/s2共12局，各6局均与模拟器结果文件目标总数核对全清。当前数据倾向方案一；每目标时间均值低11.93%，共同目标数分层等权后低5.66%，但场景未配对、样本较小，不能据此断言普遍优越。已保存中文报告、两组PNG/SVG及论文备用段落，正式测试成绩仍为空。
...
当前完成第一问的角域直径计算、第二问的可靠约束下鲁棒选点，以及第三问两套可执行基线。第二问结果属于经过数值核查的构造实验，不是连续全局最优证明；第三问已完成两方案本地对照及用户提供的12局实际演练评估。更多演练、正式测试和第四问按后续指示推进。
