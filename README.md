...
2026 年全国大学生数学建模竞赛 B 题项目。7 个原始文件已归档；第一问、第二问已完成，第三问两套方案已实现并通过本地闭环与同场景对照，用户确定使用方案一路线版。第四问进入思路审核阶段，配套中文设计、几何核算和示意图。
...
**第四问当前入口：[详细思路（待审核）](docs/第四问/详细思路_待审核.md)**。已核对全向与半平面定向混合条件，提出任意朝向的发现覆盖判据、31站三角网备选、允许失去信号的补测和顺路清除设计。现阶段只有几何分析和中文资料，尚未实现第四问运行策略或调用模拟器。

新增[圆内13站核查及最短路线](docs/第四问/圆内十三站_覆盖核查与最短路线.md)：从圆心出发、不返回时最短为11.4公里；13站存在双覆盖缺口，并可能完全漏掉朝外发射的定向源。主扫描与必要外侧补测的组合仍在讨论，配套中文路线图与反例。
...
当前完成第一问的角域直径计算、第二问的可靠约束下鲁棒选点，以及第三问两套可执行基线。第二问结果属于经过数值核查的构造实验，不是连续全局最优证明；第三问最终选用此前方案一路线版，方案二和已撤回的扇形版保留历史记录。第四问正在讨论设计，更多演练、正式测试和第四问实现按后续指示推进。
