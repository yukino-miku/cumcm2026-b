...
2026 年全国大学生数学建模竞赛 A 题项目。当前阶段：项目初始化与原始资料归档，尚未开始题意分析、建模或求解。
...
该命令会提交当前项目全部未忽略的改动，推送 `main`，并核对远端提交。没有新改动时仍会尝试推送已有本地提交。推送失败会报错，必须修复并重试后才能报告本步已同步。只读检查且没有新成果时无需空提交。
