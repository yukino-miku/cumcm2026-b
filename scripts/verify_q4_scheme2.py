"""方案二及方案一对照的独立物理审计；逐动作复核反馈、外包、费用和固定站顺序。"""
import math
import numpy as np
...
def verify_run(result):
    ...
    expected = shortest_route()[0] if '固定路线最优证书' in result else search_stations()
    assert np.allclose(result['固定站坐标'], expected, atol=1e-8, rtol=0)
    assert len(result['已完成搜索站']) == len(expected) or result['清除数'] == 16
    fixed_events = [e for e in result['动作记录'] if e['动作'] == '开始固定站']
    assert [e['固定站编号'] for e in fixed_events] == list(range(1, len(fixed_events)+1))
    assert len(fixed_events) == len(result['已完成搜索站'])
    for i, event in enumerate(fixed_events):
        assert np.allclose(event['位置'], expected[i], atol=1e-8, rtol=0)
    assert not result['配置']['discovery_safeguard']
    assert result['发现保障站坐标'] == []
    assert not result['已证明不存在频道']
    assert not any(e.get('用途') == '发现保障补扫' for e in result['动作记录'])
    for event in result['动作记录']:
        if event['动作'] == '路线规划':
            indices = [n['剩余站序'] for n in event['计划路径'] if n['类型'] == '固定站']
